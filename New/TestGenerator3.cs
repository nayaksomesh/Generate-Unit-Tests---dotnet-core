...
namespace ClassTestGenerator
{
    internal static class TestGenerator
    {
        public static string GenerateUnitTests(string sourceCode)
        {
            ...
            {
                AppendClassTests(sb, classDecl, classes, semanticModel);
            }
            ...
            SemanticModel semanticModel)
        {
            ...
            var ctorSymbols = typeSymbol?.Constructors ?? default;

            // Has parameterless ctor if there is any public instance ctor with zero params,
            // including implicit ones (IsImplicit = true). [web:77][web:88][web:94]
            ...
            var publicCtorsSyntax = classDecl.Members.OfType<ConstructorDeclarationSyntax>()
                .Where(c => c.Modifiers.Any(m => m.IsKind(SyntaxKind.PublicKeyword)))
                .ToList();
            ...
            string ctorArrange = isStaticClass
                ? string.Empty
                : GenerateCtorArrange(className, publicCtorsSyntax, allClasses, hasDefaultCtor);
            ...
            if (!isStaticClass)
            {
                sb.Append(GenerateCtorTests(className, ctorSymbols, hasDefaultCtor));
                ...
                if (!hasDefaultCtor && publicCtorsSyntax.Any())
                {
                    sb.Append(GenerateFactoryHelper(className, publicCtorsSyntax, allClasses));
                }
            }
        ...
        private static string GenerateCtorTests(
            string className,
            IEnumerable<IMethodSymbol> ctorSymbols,
            bool hasDefaultCtor)
        {
            var sb = new StringBuilder();
            ...
            // For non-default ctors, only generate tests for explicitly declared ones:
            var explicitPublicCtors = ctorSymbols
                .Where(c => !c.IsImplicit &&
                            !c.IsStatic &&
                            c.DeclaredAccessibility == Accessibility.Public &&
                            c.Parameters.Length > 0)
                .ToList();

            foreach (var ctor in explicitPublicCtors)
            {
                ...
                {
                    string pType = p.Type.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat);
                    string pName = string.IsNullOrWhiteSpace(p.Name) ? "arg" : p.Name;
        ...
        private static string GenerateCtorArrange(
            string className,
            List<ConstructorDeclarationSyntax> ctorsSyntax,
            List<ClassDeclarationSyntax> allClasses,
            bool hasDefaultCtor)
        {
            if (hasDefaultCtor)
